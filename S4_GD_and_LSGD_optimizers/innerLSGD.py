...
class loss_func(keras.Model):
            ...
            if FORWARD:
                ...
                with tf.GradientTape(watch_accessed_variables=False) as touter:
                    ...
                    with tf.GradientTape(watch_accessed_variables=False) as tinner:
                        tinner.watch(x)
                        u = self.net.call_vector(x)
                    du = tf.squeeze(tinner.batch_jacobian(u,x))
                ddu = tf.squeeze(touter.batch_jacobian(du,x))
            
            #=====================================================================
            #Construct the bilinear-form tensor
            #=====================================================================
            v = self.test(x)
            self.B = tf.einsum("kr,kn,km->mn", w, ddu, v)
    ...
    def optimal_computable_vars(self):
        
        weights_optimal = tf.linalg.lstsq(self.B, self.l, l2_regularizer=10**(-8))
...
#*********************************************************************
#  Step 1) We construct the LS system via "myloss.construct_LHMandRHV(...)"
#*********************************************************************

myloss.construct_LHMandRHV(myinputs)
checkpoint("LS-build")   

#*********************************************************************
# Step 1 BIS) We repeat the functioning of 'myloss.construct_LHMandRHV(...)' 
# so as to measure the FLOPS in each mid-checkpoint.
# Checkpoints within 'loss_func' led to inconsistent results.
#*********************************************************************
...
if STRONG:
    ...
    #=====================================================================
    v = test(x)
    Bmat = tf.einsum("kr,kj,ki->ij", w, ddu, v)
    
    checkpoint("mat-build")
    ...
    #=====================================================================
    dv = test.d(x)
    Bmat = tf.einsum("kr,kj,ki->ij", w, du, dv)
    ...
    #=====================================================================
    ddv = test.dd(x)
    Bmat = tf.einsum("kr,kj,ki->ij", w, u, ddv)
    
    checkpoint("mat-build")
    
#*********************************************************************
# Step 2) Compute the optimal coefficients
#*********************************************************************

myloss.optimal_computable_vars()
checkpoint("LS-solve")

#*********************************************************************
# Step 3) We repeat the loss evaluation and perform the traicing
#*********************************************************************
    
with tf.GradientTape(watch_accessed_variables=False) as tape:
    tape.watch(myloss.trainable_vars)
...
dout = tape.gradient(out, myloss.trainable_vars)

keras.optimizers.SGD(learning_rate=0.01).apply(dout, myloss.trainable_vars)
