...
def subprocess2measureflops(imple, N, M, K, label, error_file = ERROR_FILE):
...
flops = RESULTS_FOLDER + '/flops_LSGD.csv'
ratios = RESULTS_FOLDER + '/ratios_LSGD.csv'

headers1 = ['imple','N','M','K','net','vect-net','mat-build','LS-solve',
            'loss-trace','GD-step','total']
headers2 = ['imple','N','M','K','C=net/K','vect-net/net','mat-build/KMN',
            'LS-solve/MN2','loss-trace/KC','GD-step/loss-trace','total/KC']

with open(flops, mode='w') as file:
    writer = csv.DictWriter(file, fieldnames=headers1)
    writer.writeheader()
    
with open(ratios, mode='w') as file:
    writer = csv.DictWriter(file, fieldnames=headers1)
    writer.writeheader()
...
for i in imple_list:
    for N in N_list:
        ...
        post = subprocess2measureflops(i, N, M, K, 'LS-build')
        LSbuild = post - prev
        prev = post
        print('LS-build:  ', '{:.2e}'.format(LSbuild))
        
        post = subprocess2measureflops(i, N, M, K, 'rhs')
        rhs = post - prev
        prev = post
        print('rhs:       ', '{:.2e}'.format(rhs))
        
        post = subprocess2measureflops(i, N, M, K, 'vect-net')
        vectnet = post - prev
        prev = post
        print('vect-net:  ', '{:.2e}'.format(vectnet))
        
        post = subprocess2measureflops(i, N, M, K, 'mat-build')
        matbuild = post - prev
        prev = post
        print('mat-build: ', '{:.2e}'.format(matbuild))
        
        post = subprocess2measureflops(i, N, M, K, 'LS-solve')
        LSsolve = post - prev
        prev = post
        print('LS-solve:  ', '{:.2e}'.format(LSsolve))
        ...
        total = LSbuild + LSsolve + losstrace + GDstep
        ...
        averages_row = {'imple':i,
                        ...
                        'K':K,
                        'net':"{:.2e}".format(net),
                        'vect-net':"{:.2e}".format(vectnet),
                        'mat-build':"{:.2e}".format(matbuild),
                        'LS-solve':"{:.2e}".format(LSsolve),
        ...
        ratios_row = {'imple':i,
                      ...
                      'vect-net/net':"{:.2e}".format(vectnet/net),
                      'mat-build/KMN':"{:.2e}".format(matbuild/(K*M*N)),
                      'LS-solve/MN2':"{:.2e}".format(LSsolve/(M*N**2)),
